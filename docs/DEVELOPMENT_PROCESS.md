...
Use `docs/CI_CD_AGENT_RUNBOOK.md` for the GitHub, Netlify, and Supabase delivery checklist. GitHub CI runs the stable baseline plus deployable surface builds, local contract E2E, Deno Edge Function unit tests, scripted Supabase E2E, and Playwright browser E2E. Local clinical-readiness checks are still required when the branch depends on licensed PDFs, uploaded private Storage stimuli, installed PWA behavior, real-device browser behavior, or hosted Supabase configuration.
